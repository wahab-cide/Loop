...
const Home = () => {
  ...
  return (
    <SafeAreaView className="flex h-full items-center justify-between bg-black">
      ...
      >
        <Text className="text-[#2563EB] text-md font-InterBold">Skip</Text>
      </TouchableOpacity>
      ...
      >
        {onboarding.map((item) => (
          <View key={item.id} className="flex items-center justify-center p-5">
            ...
            <Text className="text-md font-InterSemiBold text-center text-[#B3B3B3] mx-10 mt-3">
              {item.description}
            </Text>
          </View>
        ))}
      ...
      <CustomButton
        ...
        className="w-60 px-4 py-2 mt-10 mb-5 self-center"
      />
    </SafeAreaView>
  );
};
